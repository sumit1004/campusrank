  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <motion.div variants={itemVariants} className="md:col-span-3 bg-[#111319] border border-white/5 p-8 sm:p-10 rounded-[2.5rem] relative overflow-hidden shadow-2xl group">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 relative z-10 w-full">
            <div className="space-y-4">
              ...
              <div>
                <h1 className="text-4xl sm:text-5xl font-black text-white tracking-tight italic uppercase">
                  Hello, {user?.name?.split(' ')[0]}<span className="text-indigo-500">.</span>
                </h1>
                <p className="text-sm font-bold text-gray-500 uppercase tracking-widest mt-1">Dashboard is synced and active.</p>
              </div>
              
              <div className="flex flex-wrap items-center gap-4 pt-2">
                 <div className="flex items-center bg-white/5 border border-white/5 px-4 py-2 rounded-xl text-gray-400 font-bold uppercase tracking-widest text-[10px]">
            ...
            <div className="flex gap-4 self-start md:self-auto">
               <Link to="/leaderboard" className="px-8 py-3.5 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-black rounded-2xl transition-all border border-indigo-400/20 shadow-xl shadow-indigo-600/20 uppercase tracking-[0.15em] hover:-translate-y-1">
                Leaderboard
              </Link>
            </div>
          </div>
        ...
        <motion.div variants={itemVariants} className="bg-gradient-to-br from-indigo-600 to-purple-700 p-8 rounded-[2.5rem] flex flex-col justify-center items-center text-center shadow-[0_20px_40px_rgba(79,70,229,0.3)] relative overflow-hidden group hover:scale-[1.02] transition-transform duration-300">
           <div className="absolute inset-0 bg-black/10 opacity-0 group-hover:opacity-20 transition-opacity"></div>
           <Link to="/upload" className="relative z-10 space-y-4 w-full h-full flex flex-col items-center justify-center">
              <div className="w-14 h-14 bg-white/20 rounded-2xl flex items-center justify-center mx-auto group-hover:scale-110 group-hover:rotate-6 transition-transform shadow-xl">
                 <Plus size={24} className="text-white" />
              </div>
              <div>
                <p className="text-sm font-black text-white uppercase tracking-widest">New Upload</p>
                <p className="text-[10px] font-bold text-white/60 uppercase mt-1">Manual XP Source</p>
              </div>
           </Link>
        </motion.div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* Performance Graph */}
        <motion.div variants={itemVariants} className="lg:col-span-2 bg-[#111319] border border-white/5 rounded-[3rem] p-8 sm:p-10 flex flex-col shadow-2xl relative group overflow-hidden">
          <div className="absolute -right-20 -top-20 w-64 h-64 bg-indigo-500/5 blur-[100px] rounded-full pointer-events-none"></div>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-10 gap-4">
            <div>
              ...
              <p className="text-gray-500 text-[10px] font-bold mt-1 uppercase tracking-widest">Monthly XP accumulation history</p>
            </div>
            <div className="bg-white/5 px-6 py-3 rounded-2xl border border-white/5 shadow-inner flex items-center gap-3">
               <span className="text-2xl font-black text-white">{user.total_points}</span>
               <span className="text-[9px] text-indigo-400 font-black uppercase tracking-widest px-2 py-1 bg-indigo-500/10 rounded-lg">Total XP</span>
            </div>
          </div>
                ...
                return (
                  <div key={idx} className="flex-1 flex flex-col items-center gap-4 relative z-10 group h-full justify-end">
                    <div className="w-full max-w-[48px] relative flex flex-col justify-end items-center h-full">
                      ...
                      >
                         <div className="w-full h-2 bg-white/30 rounded-full mt-1 mx-auto w-3/4"></div>
                      </motion.div>
                    </div>
        ...
        <motion.div variants={itemVariants} className="bg-[#111319] border border-white/5 p-8 rounded-[3rem] shadow-2xl flex flex-col h-[400px] lg:h-auto overflow-hidden relative">
           <div className="absolute top-0 right-0 p-8 opacity-10 pointer-events-none">
              <Clock size={100} />
           </div>
           <h4 className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.25em] flex items-center gap-3 mb-8 bg-indigo-500/10 self-start px-4 py-2 rounded-xl border border-indigo-500/20 relative z-10">
              <Clock size={14} />
              Live Activity
           </h4>
           <div className="space-y-6 overflow-y-auto custom-scrollbar flex-1 pr-4 relative z-10">
      ...
      <motion.div variants={itemVariants} className="bg-[#111319] border border-white/5 rounded-[3rem] p-8 sm:p-10 shadow-2xl relative overflow-hidden">
        <div className="absolute -left-20 -bottom-20 w-64 h-64 bg-fuchsia-500/5 blur-[100px] rounded-full pointer-events-none"></div>
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-6 relative z-10">
          <div className="space-y-1">
            ...
            <p className="text-gray-500 text-[10px] font-bold uppercase tracking-widest">Tracking verification progress of self-uploaded records</p>
          </div>
                     ...
                     manualHistory.map((m, i) => (
                        <tr key={i} className="group transition-all">
                           <td className="px-6 py-5 bg-white/[0.02] rounded-l-2xl group-hover:bg-white/[0.04]">
                              <div className="font-black text-white text-sm uppercase italic tracking-tighter truncate max-w-[250px] group-hover:text-indigo-400 transition-colors">{m.event_name}</div>
                              <div className="text-[9px] text-gray-500 font-bold uppercase tracking-widest mt-1">{m.club_name} • {m.position}</div>
                           </td>
                           ...
                           <td className="px-6 py-5 bg-white/[0.02] rounded-r-2xl group-hover:bg-white/[0.04] text-right font-black text-gray-500 text-[10px] tracking-widest">
      ...
      {eCertsCount > 0 && (
         <motion.div variants={itemVariants} className="bg-gradient-to-b from-[#111319] to-[#0d0f14] border border-white/5 rounded-[3rem] p-8 sm:p-10 shadow-2xl relative overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-10 gap-4">
               <div>
                  ...
                  <p className="text-gray-500 text-[10px] font-bold uppercase tracking-widest mt-1">Self-generating university E-Certificates</p>
               </div>
               <Link to="/profile" className="text-[10px] bg-white/5 hover:bg-white/10 border border-white/10 px-4 py-2 rounded-xl font-black text-white uppercase tracking-widest transition-colors flex items-center gap-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
               {certificates.filter(c => c.source === 'e_certificate').slice(0, 6).map((c, i) => (
                  <div key={i} className="bg-[#161a23] border border-white/5 p-6 rounded-[2rem] group hover:border-indigo-500/50 hover:bg-[#1a1f2b] transition-all relative overflow-hidden shadow-lg hover:shadow-[0_10px_30px_rgba(79,70,229,0.15)] hover:-translate-y-1">
                     ...
                     <div className="flex justify-between items-start mb-5 relative z-10">
                        <div className="p-2.5 bg-indigo-500/10 rounded-xl group-hover:scale-110 transition-transform border border-indigo-500/20">
                           <ShieldAlert size={16} className="text-indigo-400" />
                        </div>
                        <span className="text-[8px] font-black text-green-400 uppercase tracking-widest px-2 py-1 bg-green-500/10 border border-green-500/20 rounded-lg flex items-center gap-1">
                           <CheckCircle2 size={10} /> Verified
                        </span>
                     </div>
                     <h4 className="text-base font-black text-white uppercase italic tracking-tighter mb-1 truncate relative z-10">{c.event_name}</h4>
                     <p className="text-[9px] text-gray-500 font-bold uppercase mb-6 tracking-widest relative z-10">{c.position} • {new Date(c.created_at).toLocaleDateString()}</p>
                     <a href={`http://localhost:5000${c.url}`} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 w-full py-3 bg-white/5 hover:bg-indigo-600 text-white text-[10px] font-black rounded-xl transition-colors border border-white/10 hover:border-indigo-500 uppercase tracking-widest relative z-10">
                        <Download size={14} /> Output E-Cert
                     </a>
                  </div>
               ))}
            </div>
